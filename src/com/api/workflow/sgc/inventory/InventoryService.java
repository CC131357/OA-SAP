...
public class InventoryService {
    ...
    public ApiResult createApply(@Context HttpServletRequest request, @Context HttpServletResponse response, InventoryModel model) {
        ...
        try{
            ...
            //基础信息
            setBaseInfo(requestInfo,model,userModel,Utils.INVENTORYWFID);
            //main
    ...
    /**设置表单的基础信息*/
    private void setBaseInfo(WorkflowRequestInfo requestInfo, InventoryModel model,
                             UserModel userModel, String workflowId){
        ...
        baseInfo.setWorkflowId(workflowId);//流程ID
    ...
    private WorkflowRequestTableRecord[] generateDetail(InventoryModel model){//明细表添加数据方法
        ...
        for(int i=0;i<lst.size();i++){//遍历明细表1的行数，将对应的行数据插入明细表中
            ...
            tableFields[4]= Utils.generateFeild("cku",lst.get(i).getLGORT());//仓库
